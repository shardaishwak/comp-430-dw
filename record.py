...
def getTick(date: datetime = None):
    if date:
        timestamp = int(time.mktime(date.replace(minute=0, second=0, microsecond=0).timetuple()))
        where_filter = f'where: {{ pool: "0x8ad599c3a0ff1de082011efddc58f1908eb6e6d8", periodStartUnix: {timestamp} }}'
    else:
        where_filter = 'where: { pool: "0x8ad599c3a0ff1de082011efddc58f1908eb6e6d8" }'
    ...
    if not d:
        raise Exception("No poolHourData found for the given date.")
